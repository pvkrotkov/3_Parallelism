...
'''
Модифицируйте программу таким образом, чтобы одна часть программы 
генерировала случайные квадратные матрицы заданной размерности, а другая -
 перемножала их по мере генерации. Протестируйте асинхронность работы 
 программы. Реализуйте механизм остановки процесса перемножения.
'''
def matrix_gener(razmern, max_el, min_el, que):
    #global finish
    if finish:
        matric =[]
        for k in range(2):
            matric.append([])
            for i in range (razmern):
                matric[k].append([])
                for p in range (razmern):
                    matric[k][i].append(randint(min_el, max_el))
        que.put(matric)
...
def oper(que, m_cpu, numb, que_b= False):# num,
    if finish:
        if que_b:
            que = que.get()
        matrix1=que[0]    
        ...
        print('\n'+str(numb)+'\nУмножение:'+ print_table(matrix1)+ 'НА'+ print_table(matrix2)+ 'Ответ: '+str(numb)+ print_table(matric_otv)+ '\n')
    ...
    try:
        ...
        if kom[0] == 'exit':
            ...
            if lt[0] == [] or lt[1] == []:
                ...
                finish = True
                oper(lt,m_cpu, 'Из таблиц', False)# '1',
        elif kom[0] == 'genr':
            if __name__ == "__main__":
                if int(kom[3])<int(kom[4]):
                    ...
                    pool.apply_async(pusk, args=(pool, m_cpu, kom, que))#genr 1000 4 100 -100 #2
        else:
